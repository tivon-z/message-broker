 ...
 * @date 2021/12/21
 */
public interface MsgbrokerUserDetailsService extends UserDetailsService, Ordered {
	...
	default UserDetails getUserDetails(R<UserInfo> result) {
		...
		// 构造security用户
		return new MsgbrokerUser(user.getUserId(), user.getDeptId(), user.getUsername(),
	 ...
	 * @return
	 */
	default UserDetails loadUserByUser(MsgbrokerUser msgbrokerUser) {
		return this.loadUserByUsername(msgbrokerUser.getUsername());
	}
