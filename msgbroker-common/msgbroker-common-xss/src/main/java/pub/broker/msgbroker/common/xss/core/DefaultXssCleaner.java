...
package pub.broker.msgbroker.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import pub.broker.msgbroker.common.xss.config.MsgbrokerXssProperties;
import pub.broker.msgbroker.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final MsgbrokerXssProperties properties;

	public DefaultXssCleaner(MsgbrokerXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(MsgbrokerXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		MsgbrokerXssProperties.Mode mode = properties.getMode();
		if (MsgbrokerXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (MsgbrokerXssProperties.Mode.validate == mode) {
			// 校验
